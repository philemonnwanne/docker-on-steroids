...
RUN apt-get install -y --no-install-recommends tzdata ssh sudo libffi-dev systemd openssh-client && \ 
...
# Change the ownership of the user home directory '/home/vagrant/.ssh' to the default user 'vagrant'.
...
# FOR OPTIMIZATION-----------------------------------------------------------------
## (-) I minimize the number of RUN commands, as each RUN command adds a layer to the image, so consolidating the number of RUN can reduce the number of layers in the final image.
